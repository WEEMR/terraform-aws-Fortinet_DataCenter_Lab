...
resource "aws_subnet" "spoke_3_wan1_subnet" {
  count = var.create_spoke_3_vpc == "yes" ? 1 : 0
...
resource "aws_route_table_association" "spoke_3_wan1_RT_association" {
  count = var.create_spoke_3_vpc == "yes" ? 1 : 0
  provider       = aws.spoke_3_vpc
  subnet_id      = aws_subnet.spoke_3_wan1_subnet[count.index].id
...
resource "aws_route_table_association" "spoke_3_wan2_RT_association" {
  count = var.create_spoke_3_vpc == "yes" ? 1 : 0
  provider       = aws.spoke_3_vpc
  subnet_id      = aws_subnet.spoke_3_wan2_subnet[count.index].id
  route_table_id = aws_default_route_table.public_subnets_RT[count.index].id
}
...
resource "aws_route_table_association" "spoke_3_private_RT_association" {
  count = var.create_spoke_3_vpc == "yes" ? 1 : 0
  provider       = aws.spoke_3_vpc
  subnet_id      = aws_subnet.spoke_3_private_subnet[count.index].id
  route_table_id = aws_route_table.spoke_3_pvt_rt[count.index].id
}