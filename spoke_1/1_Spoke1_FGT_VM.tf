...
resource "aws_route" "spoke_1_Pvt_RT_to_Hub_1" {
  ...
  network_interface_id   = aws_network_interface.spoke_1_eth2_lan[count.index].id
...
resource "aws_instance" "spoke_1" {
  ...
  network_interface {
    network_interface_id = aws_network_interface.spoke_1_eth0_wan1[count.index].id
    device_index         = 0
  }

  network_interface {
    network_interface_id = aws_network_interface.spoke_1_eth1_wan2[count.index].id
    device_index         = 1
  }
