// hub 3 Windows Server 2019

resource "aws_network_interface" "hub3_WinSrv_eth0" {
  count = var.create_hub_3_windows == "yes" ? 1 : 0
  provider = aws.hub_3_vpc
  description = "WinSrv2019"
  subnet_id   = var.hub_3_private_subnet_id
  private_ips = var.hub_3_WinSrv_LAN_IP
  security_groups = [var.Private_SG]
  
  tags = {
    Name = "${var.username}_TF_hub_3_WinSrv"
    Owner = var.username
...
resource "aws_instance" "hub_3_WinSrv" {
  count = var.create_hub_3_windows == "yes" ? 1 : 0
  provider          = aws.hub_3_vpc
  ...
  network_interface {
    network_interface_id = aws_network_interface.hub3_WinSrv_eth0[count.index].id
    device_index         = 0
  }


  tags = {
    Name = "${var.username}_TF_hub_3_WinSrv2019"
  }
}
