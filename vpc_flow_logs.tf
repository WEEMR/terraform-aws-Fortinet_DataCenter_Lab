...
resource "aws_flow_log" "Hub2_vpc" {
  provider        = aws.virginia
  iam_role_arn    = aws_iam_role.SDWAN_Lab_IAM_Role.arn
  log_destination = aws_cloudwatch_log_group.SDWAN_Lab_VPCs_Logs.arn
  traffic_type    = "ALL"
  vpc_id          = module.hub_2_Prod_VPC.SDWAN_VPC
}
...
# Activate VPC Flow Logs on FSA

resource "aws_flow_log" "fsa1_vpc" {
  provider        = aws.virginia
  iam_role_arn    = aws_iam_role.SDWAN_Lab_IAM_Role.arn
  log_destination = aws_cloudwatch_log_group.SDWAN_Lab_VPCs_Logs.arn
  traffic_type    = "ALL"
  vpc_id          = module.FSA_1.FSA_VPC
}
*/